...
import { getServiceOfferings } from "@/app/services/offeringsServices";
...
const Page = () => {
  ...
  useEffect(() => {
    setPageTitle("Offerings");
    setAddPath(`/dashboard/groups/services/offerings/add/?uuid=${uuid}`);
    const path = `${uuid}/?limit=${limit}&page=${page}`;
    getServiceOfferings(path).then((response) => {
          ...
          <div className="">
            <div className="flex justify-between mb-4 space-x-2 ">
              ...
              {selectedGroups.length > 0 && (
                <OfferingActions uuid={selectedGroups[0]} />
              )}
            </div>
