...
import AttendanceActions from "@/app/components/attendanceActions";
...
const Page = () => {
  ...
  useEffect(() => {
    // alert(uuid);
    setPageTitle("Service Attendances");
    setAddPath(`/dashboard/groups/services/attendances/add/?uuid=${uuid}`);
    const path = `${uuid}/?limit=${limit}&page=${page}`;
    getServiceAttendances(path).then((response) => {
      setCount(response.data.body.count);
      setData(response.data.body.data);
      setLoading(false);
    });

    getAttendanceReports(uuid).then((response) => {
          ...
          <div>
            <div className="">
              <div className="flex justify-between mb-4 space-x-2 ">
                ...
                {selectedGroups.length > 0 && (
                  <AttendanceActions uuid={selectedGroups[0]} />
                )}
              </div>
              <table className="w-full">
                <thead>
                  <tr>
                    ...
                    <th className="text-start py-2 px-4">Count</th>
                  </tr>
                </thead>
                <tbody>
                  {data.map((item) => {
            ...
            <div className="mt-12">
              <TimeseriesChart
                ...
                description={
                  "This chart shows the trend of attendance over a selected period, providing insights into the changes in attendance over time."
                }
